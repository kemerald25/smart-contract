...
contract Marketplace is Ownable{
    ...
    address signer = 0xDA419a4950ba574de9Cc5c24E03B145b5D68632C;
    ...
    function enterAuction(uint256 _auctionId, uint256[] calldata _tokenIds, bytes calldata _signature) external {
        ...
        require(stakingPool.isOwnerOfStakedTokens(_tokenIds, msg.sender), "Not owner of staked tokens");
        uint256 auctionPrice = getCurrentPrice(_auctionId);
        require(token.balanceOf(msg.sender) >= auctionPrice * 1 ether, "Not enough TOKEN");
        token.burn(msg.sender, auctionPrice * 1 ether);
    ...
    function setDependencies(address _token, address _pool) external onlyOwner{
        token = TOKEN(_token);
